...
struct Item {
	...
	static constexpr uint32_t StackQuantityMask = 0x7F000000;
	static constexpr unsigned StackQuantityShift = 24;
	static constexpr uint8_t MaxStackQuantity = 99;

	[[nodiscard]] bool isStackable() const
	{
		if (isEmpty() || _itype == ItemType::Gold || _iLoc != ILOC_UNEQUIPABLE || !isUsable())
			return false;

		// Quest/special-use objects must remain individual items.
		if (IsAnyOf(_iMiscId, IMISC_MAPOFDOOM, IMISC_SPECELIX, IMISC_AURIC, IMISC_NOTE))
			return false;
		if (IsAnyOf(IDidx, IDI_MUSHROOM, IDI_FUNGALTM))
			return false;

		return true;
	}

	[[nodiscard]] uint8_t getStackQuantity() const
	{
		if (!isStackable())
			return 1;
		const uint8_t encoded = static_cast<uint8_t>((dwBuff & StackQuantityMask) >> StackQuantityShift);
		return encoded == 0 ? 1 : std::min(encoded, MaxStackQuantity);
	}

	void setStackQuantity(uint8_t quantity)
	{
		dwBuff &= ~StackQuantityMask;
		if (!isStackable() || quantity <= 1)
			return;
		quantity = std::min(quantity, MaxStackQuantity);
		dwBuff |= static_cast<uint32_t>(quantity) << StackQuantityShift;
	}
	...
	[[nodiscard]] bool canStackWith(const Item &other) const
	{
		...
		// Scroll appearance (plain/targeted) does not define the stack. The spell does.
		if (isScroll() && other.isScroll())
			return _iSpell == other._iSpell;

		// Books are also spell-specific.
		if (_iMiscId == IMISC_BOOK || other._iMiscId == IMISC_BOOK)
			return _iMiscId == IMISC_BOOK && other._iMiscId == IMISC_BOOK && _iSpell == other._iSpell;

		// All other consumables stack by consumable type. Generated item identity/seed
		// must not prevent two otherwise identical consumables from combining.
		return _iMiscId == other._iMiscId;
	}
