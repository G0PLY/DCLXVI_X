...
void PrintDebugMonster(const Monster &monster)
{
	EventPlrMsg(StrCat(
	                ...
	                "\nMonsterCount = ", ActiveMonsterCount, ", <-Active,,Max-> ,", MaxMonsters),
	    UiFlags::ColorWhite);
...
std::string ExportDun(const string_view parameter)
{
	...
	for (int y = 16; y < MAXDUNY - 16; y++) {
		for (int x = 16; x < MAXDUNX - 16; x++) {
			...
			if (dMonster[x][y] > 0) {
				for (size_t i = 0; i < MonstConvTblSize; i++) {
...
std::string DebugCmdSpawnMonster(const string_view parameter)
{
	...
	// monster definition ID, not as the normal optional spawn count.
	const bool numericMonsterId = !parameter.empty() && std::all_of(parameter.begin(), parameter.end(), [](unsigned char c) { return std::isdigit(c) != 0; });
	if (numericMonsterId) {
		const int requestedId = std::atoi(std::string(parameter).c_str());
		if (requestedId < 0 || requestedId >= NUM_MTYPES)
			return StrCat("Base monster ID out of range: ", requestedId, " (valid 0..", NUM_MTYPES - 1, ")");

		const auto mtype = static_cast<_monster_id>(requestedId);
		size_t typeIndex = MaxLvlMTypes;
		for (size_t i = 0; i < LevelMonsterTypeCount; ++i) {
			if (LevelMonsterTypes[i].type == mtype) {
				typeIndex = i;
				break;
		...
		if (typeIndex == MaxLvlMTypes) {
			if (LevelMonsterTypeCount >= MaxLvlMTypes)
				return "No free per-level monster type slot for boundary test.";
			typeIndex = LevelMonsterTypeCount++;
			CMonster &monsterType = LevelMonsterTypes[typeIndex];
			monsterType.type = mtype;
			InitMonsterGFX(monsterType);
			InitMonsterSND(monsterType);
			monsterType.placeFlags |= PLACE_SCATTER;
			monsterType.corpseId = 1;
		...
		Player &myPlayer = *MyPlayer;
		Monster *spawned = nullptr;
		Crawl(0, MaxCrawlRadius, [&](Displacement displacement) -> std::optional<bool> {
			const Point pos = myPlayer.position.tile + displacement;
			if (dPlayer[pos.x][pos.y] != 0 || dMonster[pos.x][pos.y] != 0 || !IsTileWalkable(pos))
				return {};
			spawned = AddMonster(pos, myPlayer._pdir, typeIndex, true);
			return spawned != nullptr;
		});

		if (spawned == nullptr)
			return "Unable to find space for the boundary-test monster.";

		return StrCat("Base monster boundary test: requested=", requestedId, ", level type=", static_cast<int>(LevelMonsterTypes[typeIndex].type), ", slot=", typeIndex);
	}
