...
int GetStoreSellValue(const Item &item)
{
	int value = item._ivalue;
	if (item._iMagical != ITEM_QUALITY_NORMAL && item._iIdentified)
		value = item._iIvalue;

	const int singleItemSellValue = std::max(value / 4, 1);
	const int quantity = item.isStackable() ? item.getStackQuantity() : 1;
	return singleItemSellValue * quantity;
}

std::string GetStoreSellName(const Item &item)
{
	std::string name;
	AppendStrView(name, item.getName());

	if (item.isStackable() && item.getStackQuantity() > 1)
		name = StrCat(name, " (x", static_cast<unsigned>(item.getStackQuantity()), ")");
	...
	for (int l = 5; l < 20; l += 4) {
		...
		if (!storehold[idx].isEmpty()) {
			...
			if (storehold[idx]._iMagical != ITEM_QUALITY_NORMAL && storehold[idx]._iIdentified) {
				AddSText(20, l, GetStoreSellName(storehold[idx]), itemColor, true, storehold[idx]._iCurs, true);
...
void StartSmithSell()
{
	...
	for (int8_t i = 0; i < myPlayer._pNumInv; i++) {
		...
		if (SmithSellOk(i)) {
			...
			storehold[storenumh]._ivalue = GetStoreSellValue(storehold[storenumh]);
...
void FillManaPlayer()
{
	...
	if (myPlayer._pClasstype == 1 || 2 || 3 || 4 || 7 || 9 || 10 || 11 || 12 || 13 || 14 || 15 || 16) {
...
void StoreConfirm(Item &item)
{
	...
	const bool isSellDialog = stextshold == TalkID::SmithSell || stextshold == TalkID::WitchSell;
	AddSText(20, 8, isSellDialog ? GetStoreSellName(item) : item.getName(), itemColor, false);
...
{
	storehold[storenumh] = itm;
	storehold[storenumh]._ivalue = 200;
	storehold[storenumh]._iIvalue = 200;
	storehidx[storenumh] = i;
	storenumh++;
}
