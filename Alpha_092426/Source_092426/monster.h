...
constexpr MonsterId InvalidMonsterId = std::numeric_limits<MonsterId>::max();

constexpr size_t MaxMonsters = 1024;
static_assert(MaxMonsters <= static_cast<size_t>(InvalidMonsterId), "MaxMonsters exceeds MonsterId capacity");
...
struct Monster { // note: missing field _mAFNum
	...
	static constexpr MonsterId NoLeader = InvalidMonsterId;
	 ...
	 */
	[[nodiscard]] MonsterId getId() const;
	...
	[[nodiscard]] bool isUnique() const
	{
		return uniqueType != UniqueMonsterType::None
		    && static_cast<size_t>(uniqueType) < UniqueMonsterDataCount;
	}
...
extern MonsterId ActiveMonsters[MaxMonsters];
