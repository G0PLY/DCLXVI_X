...
{
	// Lingered Shot costs exactly half the calculated mana cost of Guided Arrow.
	// Use Guided Arrow's final fixed-point mana value so both spells stay linked.
	if (sn == SpellID::SplitA)
		return GetManaAmount(player, SpellID::QuickS) / 2;
	...
	if (player.pEtherShield || (player._pClass == HeroClass::Witch && sn == SpellID::WitchBS)) { // Blood Magic
		if (sl > 0) {
			adj = 0;
		}
		...
		if (player._pClass == HeroClass::Witch && sn == SpellID::WitchBS) {
			adj = 0;
		}
...
void ConsumeSpell(Player &player, SpellID sn)
{
	...
	case SpellType::Skill:
		// Lingered Shot is currently usable through the skill path, but unlike
		// ordinary skills it intentionally consumes mana.
		if (sn == SpellID::SplitA) {
