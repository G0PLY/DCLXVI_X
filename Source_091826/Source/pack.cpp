...
{
	const bool dmgreduct = player.pDmgReduct;
	const bool ethershield = player.pEtherShield;
	const bool hlthregn = player.pHlthregn;
	const bool manaregn = player.pManaregn;
	...
	CopyUtf8(pPack->pName, player._pName, sizeof(pPack->pName));
	pPack->pClass = static_cast<uint8_t>(player._pClass);
	...
	for (int i = 0; i < NUM_INVLOC; i++) {
		...
		const bool isHellfire = netSync ? ((item.dwBuff & CF_HELLFIRE) != 0) : gbIsHellfire;
		PackItem(pPack->InvBody[i], item, isHellfire);
	}

	pPack->_pNumInv = std::min<int>(player._pNumInv, InventoryGridCells);
	...
	pPack->pManaShield = (!gbIsMultiplayer || manashield) ? (player.pManaShield ? 1 : 0) : 0;
	pPack->pDmgReduct = (!gbIsMultiplayer || dmgreduct) ? 1 : 0;
	pPack->pEtherShield = (!gbIsMultiplayer || ethershield) ? 1 : 0;
	pPack->pHlthregn = (!gbIsMultiplayer || hlthregn) ? 1 : 0;
	pPack->pManaregn = (!gbIsMultiplayer || manaregn) ? 1 : 0;
}
...
{
	const Point position { pPack->px, pPack->py };
	if (!InDungeonBounds(position))
		return false;
	if (pPack->plrlevel >= NUMLEVELS)
		return false;
	if (pPack->pClass >= enum_size<HeroClass>::value)
		return false;
	if (pPack->pLevel > MaxCharacterLevel || pPack->pLevel < 1)
		return false;
	if (pPack->_pNumInv > InventoryGridCells)
		return false;

	const uint32_t difficulty = SDL_SwapLE32(pPack->pDifficulty);
	if (difficulty > DIFF_LAST)
		return false;

	const int32_t maxHpBase = SDL_SwapLE32(pPack->pMaxHPBase);
	const int32_t hpBase = SDL_SwapLE32(pPack->pHPBase);
	if (maxHpBase < 0 || hpBase < 0 || hpBase > maxHpBase)
		return false;

	const int32_t maxManaBase = SDL_SwapLE32(pPack->pMaxManaBase);
	const int32_t manaBase = SDL_SwapLE32(pPack->pManaBase);
	if (maxManaBase < 0 || manaBase > maxManaBase)
		return false;

	const HeroClass heroClass = static_cast<HeroClass>(pPack->pClass);

	// A network player may occupy a slot that was used previously. Start from a
	// clean Player object so stale animation/GFX state cannot survive a rejoin.
	player = {};
	...
	// These fields must be valid BEFORE InitPlayer(). InitPlayer may immediately
	// select and load the stand/death animation when the remote player is on the
	// active level. The old code initialized the player while HP was still zero,
	// which incorrectly selected the death animation during multiplayer join.
	player._pMaxHPBase = maxHpBase;
	player._pHPBase = hpBase;
	player._pMaxHP = maxHpBase;
	player._pHitPoints = hpBase;
	player._pMaxManaBase = maxManaBase;
	player._pManaBase = manaBase;
	player._pMaxMana = maxManaBase;
	player._pMana = manaBase;
	...
	if (!netSync && (player._pHPBase & 0xFFFFFFC0) < 64) {
		player._pHPBase = 64;
		player._pHitPoints = std::max<int32_t>(player._pHitPoints, 64);
	}
	...
	for (int i = 0; i < NUM_INVLOC; i++) {
		const ItemPack &packedItem = pPack->InvBody[i];
		const bool isHellfire = netSync ? ((packedItem.dwBuff & CF_HELLFIRE) != 0) : (pPack->bIsHellfire != 0);
		UnPackItem(packedItem, player, player.InvBody[i], isHellfire);
	}

	player._pNumInv = pPack->_pNumInv;
	for (int i = 0; i < player._pNumInv; i++) {
		const ItemPack &packedItem = pPack->InvList[i];
		const bool isHellfire = netSync ? ((packedItem.dwBuff & CF_HELLFIRE) != 0) : (pPack->bIsHellfire != 0);
		UnPackItem(packedItem, player, player.InvList[i], isHellfire);
	}

	for (int i = 0; i < InventoryGridCells; i++) {
		const int8_t gridValue = pPack->InvGrid[i];
		if (gridValue > player._pNumInv || gridValue < -player._pNumInv)
			return false;
		player.InvGrid[i] = gridValue;
	}

	VerifyGoldSeeds(player);

	for (int i = 0; i < MaxBeltItems; i++) {
		const ItemPack &packedItem = pPack->SpdList[i];
		const bool isHellfire = netSync ? ((packedItem.dwBuff & CF_HELLFIRE) != 0) : (pPack->bIsHellfire != 0);
		UnPackItem(packedItem, player, player.SpdList[i], isHellfire);
	}

	// Recalculate equipment-derived stats and, critically, _pgfxnum after the
	// network inventory has been reconstructed.
