...
bool PlaceCryptzStairs(lvl_entry entry)
{
	...
	// Place stairs up
	position = PlaceMiniSet(currlevel != 25 ? L7STAIRSUPHF : L7STAIRSTOWN, DMAXX * DMAXY, true);
	if (!position) {
		success = false;
	} else if (entry == ENTRY_MAIN || entry == ENTRY_TWARPDN) {
		ViewPosition = position->megaToWorld() + Displacement { 3, 5 };
	...
	// Place stairs down
	if (currlevel != 28) {
		position = PlaceMiniSet(L7STAIRSDOWN, DMAXX * DMAXY, true);
		if (!position)
			success = false;
		else if (entry == ENTRY_PREV)
			ViewPosition = position->megaToWorld() + Displacement { 3, 7 };
