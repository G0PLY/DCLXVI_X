...
void DrlgTPass3()
{
	...
	//FillSector("levels\\towndata\\townh.dun", 46, 46);
	//FillSector("levels\\towndata\\townch.dun", 46, 0);
	//FillSector("levels\\towndata\\townc.dun", 0, 46);
	...
	FillSector("levels\\towndata\\townch.dun", 0, 0);
	//FillSector("levels\\towndata\\towntest.dun", 0, 0);
	//FillSector("levels\\towndata\\sector4s.dun", 0, 0);

	auto dunData = LoadFileInMem<uint16_t>("levels\\towndata\\automap.dun");
	//auto dunData = LoadFileInMem<uint16_t>("levels\\l1data\\l1.dun");
...
bool OpensGravez(Point position)
{
	int yp = position.y;
	int xp = position.x;
	return xp >= 3 && xp <= 4 && yp >= 3 && yp <= 5;
}
...
void OpenGravez()
{
	NetSendCmd(false, CMD_OPENGRAVEZ);
	//auto &quest = Quests[Q_GRAVEZ];
	//quest._qactive = QUEST_DONE;
	//if (gbIsMultiplayer)
	//	NetSendCmdQuest(true, quest);
}
