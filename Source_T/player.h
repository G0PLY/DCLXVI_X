...
constexpr int MaxCharacterLevel = 100;
...
constexpr std::array<char, 17> CharChar = {
	'w', // warrior
	'r', // rogue
	's', // sorcerer
	'm', // monk
...
struct Player {
	...
	int UpdateManaPercentage()
	{
		...
		if (_pClasstype == 1 || 2 || 3 || 4|| 7 || 9 || 10 || 11 || 12 || 13  || 14 || 15 || 16) {
			if (_pMaxMana <= 0) {
				_pManaPer = 0;
			} else {
	...
	bool CanChangeAction()
	{
		...
		if (_pmode == PM_RATTACK && AnimInfo.currentFrame >= _pAFNum)
			return true;
		if (_pmode == PM_SATTACK && AnimInfo.currentFrame >= _pAFNum)
			return true;
...
extern void Regen(Player &player);
