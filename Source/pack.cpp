...
void PackPlayer(PlayerPack &packed, const Player &player)
{
	...
	packed.pClasstype = player._pClasstype;
	...
	for (int i = 46; i < 56; i++)
		packed.pSplLvl2[i - 46] = player._pSplLvl[i];
...
void PackNetPlayer(PlayerNetPack &packed, const Player &player)
{
	...
	packed.pDmgReduct = player.pDmgReduct;
	packed.pEtherShield = player.pEtherShield;
	packed.pHlthregn = player.pHlthregn;
	packed.pManaregn = player.pManaregn;
...
void UnPackPlayer(const PlayerPack &packed, Player &player)
{
	...
	player._pClasstype = packed.pClasstype;
	...
	for (int i = 0; i < 46; i++) // Should be MAX_SPELLS but set to 36 to make save games compatible
		player._pSplLvl[i] = packed.pSplLvl[i];
	for (int i = 46; i < 56; i++)
		player._pSplLvl[i] = packed.pSplLvl2[i - 46];
...
bool UnPackNetPlayer(const PlayerNetPack &packed, Player &player)
{
	...
	player.pDmgReduct = packed.pDmgReduct != 0;
	player.pEtherShield = packed.pEtherShield != 0;
	player.pHlthregn = packed.pHlthregn != 0;
	player.pManaregn = packed.pManaregn != 0;
