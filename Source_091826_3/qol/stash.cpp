...
uint8_t MergeStashStack(Item &destination, Item &source)
{
...
void CheckStashPaste(Point cursorPosition)
{
	...
	if (stashIndex != StashStruct::EmptyCell && Stash.stashList[stashIndex].canStackWith(player.HoldItem)) {
		MergeStashStack(Stash.stashList[stashIndex], player.HoldItem);
		Stash.dirty = true;
...
void DrawStash(const Surface &out)
{
	...
	for (auto slot : StashGridRange) {
		...
		if (item.isStackable() && item.getStackQuantity() > 1) {
			DrawString(out, StrCat(static_cast<unsigned>(item.getStackQuantity())), { position - Displacement { 0, InventorySlotSizeInPixels.height }, InventorySlotSizeInPixels }, UiFlags::ColorWhite);
		}
	}
...
bool UseStashItem(uint16_t c)
{
	...
	if (Stash.stashList[c].isStackable() && Stash.stashList[c].getStackQuantity() > 1) {
		Stash.stashList[c].setStackQuantity(Stash.stashList[c].getStackQuantity() - 1);
		Stash.dirty = true;
	} else {
...
bool AutoPlaceItemInStash(Player &player, const Item &item, bool persistItem)
{
	...
	if (item.isStackable()) {
		unsigned capacity = 0;
		for (const Item &stashItem : Stash.stashList) {
			if (stashItem.canStackWith(item))
				capacity += Item::MaxStackQuantity - stashItem.getStackQuantity();
		}
		if (capacity >= item.getStackQuantity()) {
			if (persistItem) {
				Item remaining = item;
				for (Item &stashItem : Stash.stashList) {
					if (remaining.isEmpty())
						break;
					MergeStashStack(stashItem, remaining);
				}
				Stash.dirty = true;
			}
			return true;
