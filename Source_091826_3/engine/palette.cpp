...
void LoadPalette(const char *pszFileName, bool blend /*= true*/)
{
	...
	if (blend) {
		if (leveltype == DTYPE_CAVES || leveltype == DTYPE_CRYPT || leveltype == DTYPE_CRYPTZ) {
...
void LoadRndLvlPal(dungeon_type l)
{
	...
	if (l == DTYPE_TOWN) {
		...
		//LoadPalette("levels\\l1data\\l1_4.pal");
		//LoadPalette("levels\\pal\\newtown.pal");
		LoadPalette("levels\\pal\\newtown3.pal");
		return;
	}
