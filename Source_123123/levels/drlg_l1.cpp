...
bool PlaceCathedralStairs(lvl_entry entry)
{
	...
	if (DTYPE_CRYPT) {
		...
		//goto skj;
	} else if (DTYPE_CRYPTZ) {

		position = PlaceMiniSet(MyPlayer->pOriginalCathedral ? L7STAIRSUP : STAIRSUP, DMAXX * DMAXY, true);
	} else {
		position = PlaceMiniSet(MyPlayer->pOriginalCathedral ? L5STAIRSUP : STAIRSUP, DMAXX * DMAXY, true);
	}
	//skj:;
...
bool PlaceStairs(lvl_entry entry)
{
	if (leveltype == DTYPE_CRYPT) {
		...
		return PlaceCryptzStairs(entry);
	} else {
...
void CreateL7Dungeon(uint32_t rseed, lvl_entry entry)
{
	SetRndSeed(rseed);
...
void LoadL1Dungeon(const char *path, Point spawn)
{
	...
	if (setlvltype == DTYPE_CRYPT) {
		...
		AddCryptzObjects(0, 0, MAXDUNX, MAXDUNY);
		PlaceCryptLights();
	} else {
