...
/**
 * @brief Returns SDL's millisecond clock with sub-millisecond precision.
 *
 * The authoritative game/network scheduler continues to use SDL_GetTicks().
 * This clock is used only for render interpolation so higher refresh-rate
 * displays are not limited by the 1 ms granularity of SDL_GetTicks().
 */
double GetHighResolutionTickCount()
{
#ifdef USE_SDL1
	return SDL_GetTicks();
#else
	static const uint64_t PerformanceFrequency = SDL_GetPerformanceFrequency();
	static const uint64_t PerformanceCounterBase = SDL_GetPerformanceCounter();
	static const uint32_t TickBase = SDL_GetTicks();

	if (PerformanceFrequency == 0)
		return SDL_GetTicks();

	const uint64_t counter = SDL_GetPerformanceCounter();
	const double elapsedMilliseconds = static_cast<double>(counter - PerformanceCounterBase) * 1000.0 / static_cast<double>(PerformanceFrequency);
	return static_cast<double>(TickBase) + elapsedMilliseconds;
#endif
}
...
void nthread_UpdateProgressToNextGameTick()
{
	if (!gbRunGame || PauseMode != 0 || (!gbIsMultiplayer && gmenu_is_active()) || !gbProcessPlayers || demo::IsRunning()) // if game is not running or paused there is no next gametick in the near future
		return;
	const double currentTickCount = GetHighResolutionTickCount();
	const double ticksMissing = static_cast<double>(last_tick) - currentTickCount;
	if (ticksMissing <= 0.0) {
		ProgressToNextGameTick = AnimationInfo::baseValueFraction; // game tick is due
		return;
	}
	const double ticksAdvanced = static_cast<double>(gnTickDelay) - ticksMissing;
	int32_t fraction = static_cast<int32_t>(ticksAdvanced * AnimationInfo::baseValueFraction / gnTickDelay);
