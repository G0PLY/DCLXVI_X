...
void PkwareBufferWrite(char *buf, unsigned int *size, void *param) // NOLINT(readability-non-const-parameter)
{
	...
	const uint32_t bytesToWrite = *size;
	if (pInfo->destOffset > pInfo->destCapacity || bytesToWrite > pInfo->destCapacity - pInfo->destOffset) {
		pInfo->overflow = true;
		return;
	}

	memcpy(pInfo->destData + pInfo->destOffset, buf, bytesToWrite);
	pInfo->destOffset += bytesToWrite;
}
...
uint32_t PkwareCompress(byte *srcData, uint32_t size)
{
	...
	param.destOffset = 0;
	param.size = size;
	param.destCapacity = destSize;
	param.overflow = false;
