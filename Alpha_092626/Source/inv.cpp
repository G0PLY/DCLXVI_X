...
uint8_t MergeStack(Item &destination, Item &source)
{
	if (!destination.canStackWith(source))
		return 0;
	const uint8_t destinationQuantity = destination.getStackQuantity();
	const uint8_t sourceQuantity = source.getStackQuantity();
	if (destinationQuantity >= Item::MaxStackQuantity)
		return 0;
	const uint8_t moved = std::min<uint8_t>(sourceQuantity, Item::MaxStackQuantity - destinationQuantity);
	destination.setStackQuantity(destinationQuantity + moved);
	if (moved == sourceQuantity)
		source.clear();
	else
		source.setStackQuantity(sourceQuantity - moved);
	return moved;
}

bool CanMergeEntireStack(const Item &source, const Item *begin, const Item *end)
{
	if (!source.isStackable())
		return false;
	unsigned capacity = 0;
	for (const Item *item = begin; item != end; ++item) {
		if (item->canStackWith(source))
			capacity += Item::MaxStackQuantity - item->getStackQuantity();
	}
	return capacity >= source.getStackQuantity();
}

bool MergeEntireStack(Item source, Item *begin, Item *end)
{
	if (!CanMergeEntireStack(source, begin, end))
		return false;
	for (Item *item = begin; item != end && !source.isEmpty(); ++item)
		MergeStack(*item, source);
	return source.isEmpty();
}

void SendInventoryStackUpdate(Player &player, int invListIndex)
{
	if (&player != MyPlayer)
		return;
	for (int gridIndex = 0; gridIndex < InventoryGridCells; ++gridIndex) {
		if (abs(player.InvGrid[gridIndex]) - 1 == invListIndex) {
			NetSendCmdChInvItem(false, gridIndex);
			return;
		}
	}
}

void ConsumeOneInventoryItem(Player &player, int invListIndex)
{
	Item &item = player.InvList[invListIndex];
	if (item.isStackable() && item.getStackQuantity() > 1) {
		item.setStackQuantity(item.getStackQuantity() - 1);
		SendInventoryStackUpdate(player, invListIndex);
		player.CalcScrolls();
		return;
	}
...
void ConsumeOneBeltItem(Player &player, int beltIndex)
{
	...
	player.RemoveSpdBarItem(beltIndex);
}

} // namespace

void ConsumeInventoryItemAtIndex(Player &player, int invListIndex)
{
	ConsumeOneInventoryItem(player, invListIndex);
}

void ConsumeBeltItemAtIndex(Player &player, int beltIndex)
{
	ConsumeOneBeltItem(player, beltIndex);
}
...
bool CanWield(Player &player, const Item &item)
{
	...
	if (player._pClass == HeroClass::Bard || player._pClass == HeroClass::Assassin || player._pClass == HeroClass::Templar || player._pClass == HeroClass::Battlemage) {
...
void CheckInvPaste(Player &player, Point cursorPosition)
{
	...
	case ILOC_ONEHAND: {
		...
		bool pasteIntoSelectedHand = (player.InvBody[otherHand].isEmpty() || player.InvBody[otherHand]._iClass != player.HoldItem._iClass)
		    ...
		    || (player._pClass == HeroClass::Assassin && player.InvBody[otherHand]._iClass == ICLASS_WEAPON && player.HoldItem._iClass == ICLASS_WEAPON)
		    || (player._pClass == HeroClass::Templar && player.InvBody[otherHand]._iClass == ICLASS_WEAPON && player.HoldItem._iClass == ICLASS_WEAPON)
		    || (player._pClass == HeroClass::Battlemage && player.InvBody[otherHand]._iClass == ICLASS_WEAPON && player.HoldItem._iClass == ICLASS_WEAPON);
	...
	case ILOC_UNEQUIPABLE:
		if (it != 0 && player.HoldItem.isStackable()) {
			const int invIndex = it - 1;
			if (player.InvList[invIndex].canStackWith(player.HoldItem)) {
				MergeStack(player.InvList[invIndex], player.HoldItem);
				SendInventoryStackUpdate(player, invIndex);
				break;
			}
		}
	...
	case ILOC_BELT: {
		...
		if (!player.SpdList[ii].isEmpty() && player.SpdList[ii].canStackWith(player.HoldItem)) {
			MergeStack(player.SpdList[ii], player.HoldItem);
...
void DrawInv(const Surface &out)
{
	...
	for (int j = 0; j < InventoryGridCells; j++) {
		if (myPlayer.InvGrid[j] > 0) { // first slot of an item
			...
			if (myPlayer.InvList[ii].isStackable() && myPlayer.InvList[ii].getStackQuantity() > 1) {
				DrawString(out, StrCat(static_cast<unsigned>(myPlayer.InvList[ii].getStackQuantity())), { position - Displacement { 0, InventorySlotSizeInPixels.height }, InventorySlotSizeInPixels }, UiFlags::ColorWhite);
			}
		}
	}
}
...
void DrawInvBelt(const Surface &out)
{
	...
	for (int i = 0; i < MaxBeltItems; i++) {
		...
		if (myPlayer.SpdList[i].isStackable() && myPlayer.SpdList[i].getStackQuantity() > 1) {
			DrawString(out, StrCat(static_cast<unsigned>(myPlayer.SpdList[i].getStackQuantity())), { position - Displacement { 0, InventorySlotSizeInPixels.height }, InventorySlotSizeInPixels }, UiFlags::ColorWhite);
		}
...
bool AutoPlaceItemInBelt(Player &player, const Item &item, bool persistItem)
{
	...
	if (item.isStackable() && CanMergeEntireStack(item, player.SpdList, player.SpdList + MaxBeltItems)) {
		if (persistItem) {
			Item remaining = item;
			for (size_t i = 0; i < MaxBeltItems && !remaining.isEmpty(); ++i) {
				if (MergeStack(player.SpdList[i], remaining) != 0 && &player == MyPlayer)
...
{
	if (item.isStackable() && CanMergeEntireStack(item, player.InvList, player.InvList + player._pNumInv)) {
		if (persistItem) {
			Item remaining = item;
			for (int i = 0; i < player._pNumInv && !remaining.isEmpty(); ++i) {
				if (MergeStack(player.InvList[i], remaining) != 0)
					SendInventoryStackUpdate(player, i);
			}
			player.CalcScrolls();
		}
		return true;
...
int8_t CheckInvHLight()
{
	...
	if (pi->_itype == ItemType::Gold) {
		...
		if (pi->isStackable() && pi->getStackQuantity() > 1)
			InfoString = StrCat(InfoString, " (x", static_cast<unsigned>(pi->getStackQuantity()), ")");
		if (pi->_iIdentified) {
			PrintItemDetails(*pi);
		} else {
			PrintItemDur(*pi);
...
void ConsumeScroll(Player &player)
{
	...
	if (itemSlot >= INVITEM_INV_FIRST && itemSlot <= INVITEM_INV_LAST) {
		...
		if (!item->isEmpty() && isCurrentSpell(*item)) {
			ConsumeOneInventoryItem(player, itemIndex);
			return;
		}
		...
		if (!item->isEmpty() && isCurrentSpell(*item)) {
			ConsumeOneBeltItem(player, itemIndex);
			return;
		}
	} else if (itemSlot != 0) {
...
bool UseInvItem(int cii)
{
	...
	if (!item->isScroll() && !item->isRune())
		ConsumeOneInventoryItem(player, c);
