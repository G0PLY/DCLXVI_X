...
/**
 * @brief Tracks tiles that contain at least one missile at its interpolated render position.
 *
 * DrawDungeon visits roughly the whole visible tile set every frame. Most of those tiles do
 * not contain missiles, so avoid an unordered_multimap lookup for the common empty-tile case.
 */
Bitset2d<MAXDUNX, MAXDUNY> MissileRenderingTiles;
...
void UpdateMissilesRendererData()
{
	...
	MissileRenderingTiles.reset();
	//MissileRenderingTiles.clear();

	for (auto &m : Missiles) {
		UpdateMissileRendererData(m);
		MissilesAtRenderingTile.insert(std::make_pair(m.position.tileForRendering, &m));
		if (InDungeonBounds(m.position.tileForRendering))
			MissileRenderingTiles.set(m.position.tileForRendering.x, m.position.tileForRendering.y);
	}
}
 ...
 */
bool DrawMissilePrivate(const Surface &out, const Missile &missile, Point targetBufferPosition, bool pre)
{
...
{
	if (!MissileRenderingTiles.test(tilePosition.x, tilePosition.y))
		return;
...
void DrawPlayerIcons(const Surface &out, const Player &player, Point position, bool infraVision)
{
	if (player.pManaShield) {
		DrawPlayerIconHelper(out, MissileGraphicID::ManaShield, position, &player != MyPlayer, infraVision);  // 0, -8+ Displacement { 48, -64 }
	}
	/*if (player.pEtherShield) {
		if (player._pClass == HeroClass::Paladin) {
			DrawPlayerIconHelper(out, MissileGraphicID::HealthRegen, position + Displacement { 0, 0 }, &player != MyPlayer, infraVision);
		} else {
			DrawPlayerIconHelper(out, MissileGraphicID::ManaRegen, position + Displacement { 0, 0 }, &player != MyPlayer, infraVision);
		}
	}*/
	if (player.pDmgReduct) {
		DrawPlayerIconHelper(out, MissileGraphicID::DmgReduct, position + Displacement { 0, -8 }, &player != MyPlayer, infraVision);// 0, -8
	}

	if (player.pHlthregn) {
		DrawPlayerIconHelper(out, MissileGraphicID::HealthRegen, position + Displacement { 0, -16 }, &player != MyPlayer, infraVision);// 0, -16
	}
...
void DrawPlayer(const Surface &out, const Player &player, Point tilePosition, Point targetBufferPosition)
{
	...
	if (&player == MyPlayer && IsNoneOf(leveltype, DTYPE_NEST, DTYPE_CRYPT, DTYPE_CRYPTZ)) {
			...
			if (levelCelBlock.hasValue()) {
				if (transparency || !foliage || levelCelBlock.type() == TileType::TransparentSquare) {
					if (maskType != MaskType::RightFoliage || tileType == TileType::TransparentSquare) {
						const Point renderPosition = targetBufferPosition + Displacement { TILE_WIDTH / 2, 0 };
						RenderTile(out, renderPosition,
						    levelCelBlock, maskType, tbl);
					}
				}
			}
	...
	for (uint_fast8_t i = 2, n = MicroTileLen; i < n; i += 2) {
		{
			...
			if (levelCelBlock.hasValue()) {
				const MaskType maskType = transparency ? MaskType::Transparent : MaskType::Solid;
				RenderTile(out, targetBufferPosition,
				    levelCelBlock, maskType, tbl);
			}
		}
		...
		for (int j = 0; j < columns; j++) {
			if (InDungeonBounds(tilePosition)) {
				...
				if (tilePosition.x + 1 < MAXDUNX && tilePosition.y - 1 >= 0 && targetBufferPosition.x + TILE_WIDTH <= gnScreenWidth) {
					...
					// A proper fix for this would probably have been to layout the screen and render by
					// sprite screen position rather than tile position.
					const bool currentIsWall = IsWall(tilePosition);
					if (currentIsWall) {
						bool alignedWall = IsWall(tilePosition + Displacement { 1, 0 });
						if (!alignedWall && tilePosition.x > 0) {
							alignedWall = IsWall(tilePosition + Displacement { -1, 0 });
						}
						if (alignedWall) {
							const bool eastNorthWalkable = IsTileNotSolid(tilePosition + Displacement { 1, -1 });
							if (eastNorthWalkable) {
								if (IsTileNotSolid(tilePosition + Displacement { 0, -1 })) {
...
void DrawFPS(const Surface &out)
{
	...
	if (msSinceLastUpdate >= 1000) {
		...
		//formattedd = std::to_string(ActiveMonsterCount);
		...
		formatted = { buf, static_cast<string_view::size_type>(end - buf)  };
		    //? BufCopy(buf, fps / FpsPow10, " FPS + Monsters " + std::to_string(ActiveMonsterCount))
		   // : BufCopy(buf, fps / FpsPow10, ".", fps % FpsPow10, " FPS + Monsters " + std::to_string(ActiveMonsterCount));

		
		//static char buff[3] {};
		//const char *end = fps >= 100 * FpsPow10
		//    ? BufCopy(buff, ActiveMonsterCount, " FPS")
		//: BufCopy(buf, ActiveMonsterCount, ".", fps % FpsPow10, " FPS");
	};
	DrawString(out, formatted, Point { 8, 68 }, UiFlags::ColorRed);
	//	DrawString(out, formattedd, Point { 5, 68 }, UiFlags::ColorRed);
}
