...
void LimitFrameRate()
{
	if (!*sgOptions.Graphics.limitFPS || refreshDelay <= 0)
		return;

#ifdef USE_SDL1
	// SDL1 has no high-resolution performance-counter API. Preserve the
	// existing limiter on legacy builds.
	...
	// refreshDelay is expressed in microseconds. Keep the existing target rate,
	// but pace against SDL's high-resolution monotonic performance counter.
	const uint64_t frameInterval = std::max<uint64_t>(1, static_cast<uint64_t>(refreshDelay) * frequency / 1000000);
	static uint64_t frameDeadline = 0;
	uint64_t now = SDL_GetPerformanceCounter();

	if (frameDeadline == 0 || now >= frameDeadline + frameInterval) {
		// First frame, or we fell more than one frame behind: restart from now
		// rather than trying to catch up with a burst of frames.
		frameDeadline = now + frameInterval;
		return;
	}

	if (now < frameDeadline) {
		const uint64_t remaining = frameDeadline - now;
		const uint32_t remainingMs = static_cast<uint32_t>(remaining * 1000 / frequency);

		// SDL_Delay has millisecond granularity. Sleep most of the interval, then
		// use the performance counter for the final fraction of a millisecond.
		if (remainingMs > 1)
			SDL_Delay(remainingMs - 1);
