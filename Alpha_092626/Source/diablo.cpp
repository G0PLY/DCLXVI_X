...
bool StartGame(bool bNewGame, bool bSinglePlayer)
{
	...
	do {
		...
		giNumberOfLevels = gbIsHellfire ? 29 : 17;  //25 : 17
...
void diablo_color_cyc_logic()
{
	...
	if (leveltype == DTYPE_CAVES) {
		...
		// DCLXVI Build 11: keep the Hell blood-wall lighting tables stable.
		// MakeLightTable() already installs Hell-specific blood-wall translations. Rotating
		// entries 1..31 in every LightTables row each game-logic cycle also changes the
		// translation used by ordinary terrain pixels in that palette range. Runtime A/B
		// testing in Build 10 confirmed that this mutation caused the main Hell tile flicker.
		// Leave the Hell translations static rather than mutating shared terrain light tables.
	} else if (leveltype == DTYPE_NEST) {
		palette_update_hive();
	} else if (leveltype == DTYPE_CRYPT) {
		palette_update_crypt();
	} else if (leveltype == DTYPE_CRYPTZ) {
		palette_update_cryptz();
	}
}
