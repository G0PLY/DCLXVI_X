...
inline void ZtJoinLifeDiag(const char *message)
{
...
{
	ZtJoinLifeDiag("[34D JOIN] wait_network begin");
	...
	ZtJoinLifeDiag(online ? "[34D JOIN] wait_network final ONLINE" : "[34D JOIN] wait_network TIMEOUT/OFFLINE");
	return online;
}
...
{
	ZtJoinLifeDiag("[34D DISCOVERY] wait_firstpeer begin");
	for (auto i = 0; i < 500; ++i) {
	...
	ZtJoinLifeDiag(firstpeer ? "[34D DISCOVERY] matching game FOUND" : "[34D DISCOVERY] matching game TIMEOUT");
...
{
	ZtJoinLifeDiag("[34D JOIN] sending PT_JOIN_REQUEST");
	...
	ZtJoinLifeDiag(plr_self != PLR_BROADCAST ? "[34D JOIN] PT_JOIN_ACCEPT completed" : "[34D JOIN] PT_JOIN_ACCEPT TIMEOUT");
}
...
{
	ZtJoinLifeDiag("[34D JOIN] join requested");
	gamename = addrstr;
	isGameHost_ = false;
	...
	ZtJoinLifeDiag(plr_self == PLR_BROADCAST ? "[34D JOIN] join returning FAILURE" : "[34D JOIN] join returning SUCCESS");
	return (plr_self == PLR_BROADCAST ? -1 : plr_self);
}
