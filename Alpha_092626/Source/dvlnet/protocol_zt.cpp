...
namespace {
void ZtLifeDiag(const char *message)
{
...
{
	ZtLifeDiag("[34D PROTOCOL] construct -> zerotier_network_start");
...
bool protocol_zt::network_online()
{
	...
	if (fd_udp == -1) {
		ZtLifeDiag("[34D SOCKET] creating UDP socket");
		...
		set_nonblock(fd_udp);
		ZtLifeDiag("[34D SOCKET] UDP bind OK");
	}
	if (fd_tcp == -1) {
		ZtLifeDiag("[34D SOCKET] creating TCP socket");
		...
		set_nonblock(fd_tcp);
		set_nodelay(fd_tcp);
		ZtLifeDiag("[34D SOCKET] TCP bind/listen OK");
	}
	return true;
}
...
protocol_zt::~protocol_zt()
{
	ZtLifeDiag("[34D PROTOCOL] destruct -> close_all (ZeroTier runtime remains active)");
	close_all();
}
