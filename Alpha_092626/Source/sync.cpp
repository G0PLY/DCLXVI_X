...
void SyncMonsterPos(TSyncMonster &monsterSync, MonsterId ndx)
{
	auto &monster = Monsters[ndx];
	monsterSync._mndx = SDL_SwapLE16(ndx);
	monsterSync._mx = monster.position.tile.x;
	monsterSync._my = monster.position.tile.y;
	monsterSync._menemy = SDL_SwapLE16(static_cast<uint16_t>(encode_enemy(monster)));
...
bool SyncMonsterActive2(TSyncMonster &monsterSync)
{
	...
	//	for (int i = 0; i < ActiveMonsterCount; i++) {
		...
		MonsterId m = ActiveMonsters[sgnMonsters];
		if (sgwLRU[m] < lru) {
			lru = sgwLRU[m];
...
void SyncMonster(bool isOwner, const TSyncMonster &monsterSync)
{
	...
	const int enemyId = SDL_SwapLE16(monsterSync._menemy);
...
bool IsTSyncMonsterValidate(const TSyncMonster &monsterSync)
{
	...
	if (!IsEnemyIdValid(Monsters[monsterId], SDL_SwapLE16(monsterSync._menemy)))
		return false;
...
uint32_t sync_all_monsters(byte *pbBuf, uint32_t dwMaxLen)
{
	...
	// Preserve the two LRU-selected entries at the head of the packet. These are
	// intentionally chosen exactly as before, including the cyclic ActiveMonsters
	// tie-breaking performed by SyncMonsterActive2().
	for (size_t i = 0; i < 2 && dwMaxLen >= sizeof(TSyncMonster); i++) {
		auto &monsterSync = *reinterpret_cast<TSyncMonster *>(pbBuf);
		if (!SyncMonsterActive2(monsterSync))
			break;
	...
	// The old SyncMonsterActive() rescanned every active monster for every
	// remaining packet slot. At the 1024-monster ceiling that turns one bounded
	// sync packet into dozens of full population scans. Build the same eligible
	// sequence once instead. stable_sort preserves ActiveMonsters order for equal
	// priorities, matching the old strict-< tie behavior.
	std::array<MonsterId, MaxMonsters> priorityOrder {};
	size_t priorityCount = 0;
	for (size_t i = 0; i < ActiveMonsterCount; i++) {
		const MonsterId monsterId = ActiveMonsters[i];
		if (sgwLRU[monsterId] < 0xFFFE)
			priorityOrder[priorityCount++] = monsterId;
	}

	std::stable_sort(priorityOrder.begin(), priorityOrder.begin() + priorityCount, [](MonsterId a, MonsterId b) {
		return sgnMonsterPriority[a] < sgnMonsterPriority[b];
	});

	for (size_t i = 0; i < priorityCount && dwMaxLen >= sizeof(TSyncMonster); i++) {
		auto &monsterSync = *reinterpret_cast<TSyncMonster *>(pbBuf);
		SyncMonsterPos(monsterSync, priorityOrder[i]);
		pbBuf += sizeof(TSyncMonster);
		pHdr->wLen += sizeof(TSyncMonster);
		dwMaxLen -= sizeof(TSyncMonster);
	}
...
uint32_t OnSyncData(const TCmd *pCmd, size_t pnum, size_t availableBytes)
{
	if (availableBytes < sizeof(TSyncHeader)) {
		return 0;
	}

	const auto &header = *reinterpret_cast<const TSyncHeader *>(pCmd);
	const uint16_t wLen = SDL_SwapLE16(header.wLen);
	if (wLen % sizeof(TSyncMonster) != 0) {
		return 0;
	}
	if (wLen > availableBytes - sizeof(header)) {
		return 0;
	}
	...
	const int monsterCount = wLen / sizeof(TSyncMonster);
