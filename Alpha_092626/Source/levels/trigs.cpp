...
const uint16_t TownCryptzList[] = { 1330, 1331, 1332, 1333, 1334, 1335, 1336, 1337 };
...
const uint16_t L7TWarpUpList[] = { 78, 79, 80, 81, 82, 83, 84, 85, 86, 87, 88, 89, 90, 91 };
const uint16_t L7UpList[] = { 64, 65, 66, 67, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77 };
const uint16_t L7DownList[] = { 56, 57, 58, 59, 60, 61, 62, 63 };
} // namespace
...
bool IsWarpOpen(dungeon_type type)
{
	...
	if (gbIsHellfire) {
		...
		if (type == DTYPE_CRYPT && Quests[Q_GRAVE]._qactive == QUEST_DONE)
			return true;
		if (type == DTYPE_CRYPTZ && myPlayer._pLevel >= 30)
			return true;
	}
...
void InitTownTriggers()
{
	...
	if (IsWarpOpen(DTYPE_CRYPTZ)) {
		trigs[numtrigs].position = { 3, 3 };
		trigs[numtrigs]._tmsg = WM_DIABTOWNWARP;
...
void InitHiveTriggers()
{
	...
	for (WorldTileCoord j = 0; j < MAXDUNY; j++) {
		for (WorldTileCoord i = 0; i < MAXDUNX; i++) {
			if (dPiece[i][j] == 65 && numtrigs < MAXTRIGGERS) {
				trigs[numtrigs].position = { i, j };
				trigs[numtrigs]._tmsg = WM_DIABPREVLVL;
				numtrigs++;
			}

			if (dPiece[i][j] == 62 && numtrigs < MAXTRIGGERS) {
				trigs[numtrigs].position = { i, j };
				trigs[numtrigs]._tmsg = WM_DIABNEXTLVL;
				numtrigs++;
			}

			if (dPiece[i][j] == 79 && numtrigs < MAXTRIGGERS) {
				trigs[numtrigs].position = { i, j };
				trigs[numtrigs]._tmsg = WM_DIABTWARPUP;
				numtrigs++;
			}
...
bool ForceTownTrig()
{
	...
	if (IsWarpOpen(DTYPE_CRYPTZ)) {
		for (const uint16_t tileId : TownCryptzList) {
			if (dPiece[cursPosition.x][cursPosition.y] == tileId) {
				InfoString = _("Down to Cryptz");
				cursPosition = { 3, 4 };
				return true;
			}
		}
...
bool ForceL4Trig()
{
	...
	if (currlevel == 15 && Quests[Q_BETRAYER]._qactive == QUEST_DONE) {
...
bool ForceCryptzTrig()
{
	for (const uint16_t tileId : L7UpList) {
		if (dPiece[cursPosition.x][cursPosition.y] == tileId) {
		//	InfoString = fmt::format(fmt::runtime(_("Up to Cryptz level {:d}")), currlevel - 25);
			for (int j = 0; j < numtrigs; j++) {
				if (trigs[j]._tmsg == WM_DIABPREVLVL) {
					cursPosition = trigs[j].position;
					return true;
	...
	//	return true;
	//}
	for (const uint16_t tileId : L7DownList) {
		if (dPiece[cursPosition.x][cursPosition.y] == tileId) {
			//InfoString = fmt::format(fmt::runtime(_("Down to Cryptz level {:d}")), currlevel - 23);
			for (int j = 0; j < numtrigs; j++) {
				if (trigs[j]._tmsg == WM_DIABNEXTLVL) {
					cursPosition = trigs[j].position;
					return true;
	...
	if (currlevel == 25) {
		for (const uint16_t tileId : L7TWarpUpList) {
			if (dPiece[cursPosition.x][cursPosition.y] == tileId) {
				for (int j = 0; j < numtrigs; j++) {
					if (trigs[j]._tmsg == WM_DIABTWARPUP) {
						int dx = abs(trigs[j].position.x - cursPosition.x);
...
bool ForceArenaTrig()
{
	...
	case DTYPE_CRYPT:
		...
		break;
	case DTYPE_CRYPTZ:
		checkList = L7TWarpUpList;
		len = sizeof(L7TWarpUpList) / sizeof(L7TWarpUpList[0]);
		break;
	default:
		return false;
	}
	...
	for (int i = 0; i < numtrigs; i++) {
		...
		case WM_DIABTOWNWARP:
			if (gbIsMultiplayer) {
				...
				if (IsAnyOf(trigs[i]._tlvl, 13, 25) && myPlayer._pLevel < 17) {
					abort = true;
					position.y += 1;
