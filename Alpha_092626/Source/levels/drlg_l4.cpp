...
void FixTilesPatterns()
{
	...
	for (int j = 2; j < DMAXY - 1; j++) {
		for (int i = 1; i < DMAXX - 2; i++) {
...
void FixTransparency()
{
	...
	for (int j = 0; j < DMAXY; j++) {
		int xx = 16;
		for (int i = 0; i < DMAXX; i++) {
			// The north-neighbor test is only valid away from the top edge.
			if (j > 0 && IsDURightWall(dungeon[i][j]) && dungeon[i][j - 1] == 18) {
				dTransVal[xx + 1][yy] = dTransVal[xx][yy];
				dTransVal[xx + 1][yy + 1] = dTransVal[xx][yy];
			}
			// The east-neighbor test is only valid away from the right edge.
			if (i + 1 < DMAXX && IsDLLeftWall(dungeon[i][j]) && dungeon[i + 1][j] == 19) {
