...
string_view CmdIdString(_cmd_id cmd)
{
	...
	case CMD_ACTIVATERPORTAL: return "CMD_ACTIVATERPORTAL";
	case CMD_DEACTIVATERPORTAL: return "CMD_DEACTIVATERPORTAL";
	...
	case CMD_SETDRSHIELD: return "CMD_SETDRSHIELD";
	case CMD_SETESHIELD: return "CMD_SETESHIELD";
	case CMD_SETHRSHIELD: return "CMD_SETHRSHIELD";
	case CMD_SETMRSHIELD: return "CMD_SETMRSHIELD";
	case CMD_REMSHIELD: return "CMD_REMSHIELD";
	case CMD_REDRSHIELD: return "CMD_REDRSHIELD";
	case CMD_REMESHIELD: return "CMD_REMESHIELD";
...
void PrePacket()
{
	...
	for (TMegaPkt &pkt : MegaPktList) {
		...
		while (spaceLeft != pkt.spaceLeft) {
			const size_t available = spaceLeft - pkt.spaceLeft;
			if (available < sizeof(TCmd)) {
				Log("Discarding truncated buffered network message");
				return;
			}
			...
			if (cmdId == FAKE_CMD_SETID) {
				if (available < sizeof(TFakeCmdPlr)) {
					Log("Discarding truncated buffered player-id message");
					return;
				}
			...
			if (cmdId == FAKE_CMD_DROPID) {
				if (available < sizeof(TFakeDropPlr)) {
					Log("Discarding truncated buffered player-drop message");
					return;
				}
			...
			size_t size = ParseCmd(playerId, (TCmd *)data, available);
...
bool DeltaImportItem(const byte *&src, const byte *end, TCmdPItem *dst)
{
	for (int i = 0; i < MAXITEMS; i++, dst++) {
		...
		if (*src == byte { 0xFF }) {
			...
			if (static_cast<size_t>(end - src) < sizeof(TCmdPItem))
				return false;
			memcpy(dst, src, sizeof(TCmdPItem));
...
bool DeltaImportObjects(const byte *&src, const byte *end, std::unordered_map<WorldTilePosition, DObjectStr> &dst)
{
	if (src >= end)
		return false;

	const uint8_t numDeltas = static_cast<uint8_t>(*src++);
	if (numDeltas > MAXOBJECTS || static_cast<size_t>(end - src) < static_cast<size_t>(numDeltas) * 3)
		return false;
...
bool DeltaImportMonster(const byte *&src, const byte *end, DMonsterStr *dst)
{
	for (size_t i = 0; i < MaxMonsters; i++, dst++) {
		...
		if (*src == byte { 0xFF }) {
			...
			if (static_cast<size_t>(end - src) < sizeof(DMonsterStr))
				return false;
			memcpy(dst, src, sizeof(DMonsterStr));
...
bool DeltaImportJunk(const byte *&src, const byte *end, DJunk &junk)
{
	for (int i = 0; i < MAXPORTAL; i++) {
		if (src >= end)
			return false;
		if (*src == byte { 0xFF }) {
			memset(&junk.portal[i], 0xFF, sizeof(DPortal));
			src++;
		} else {
			if (static_cast<size_t>(end - src) < sizeof(DPortal))
				return false;
			memcpy(&junk.portal[i], src, sizeof(DPortal));
			src += sizeof(DPortal);
		}
	}
	...
	for (int qidx = 0; qidx < MAXQUESTS; qidx++) {
		if (QuestsData[qidx].isSinglePlayerOnly && UseMultiplayerQuests())
			continue;
		if (static_cast<size_t>(end - src) < sizeof(MultiQuests))
			return false;
		memcpy(&junk.quests[q], src, sizeof(MultiQuests));
...
bool DeltaImportData(_cmd_id cmd, uint32_t recvOffset)
{
	if (recvOffset <= 1 || recvOffset > sizeof(sgRecvBuf))
		return false;

	uint32_t dataSize = recvOffset - 1;
#ifdef USE_PKWARE
	if (sgRecvBuf[0] != byte { 0 }) {
		dataSize = PkwareDecompress(&sgRecvBuf[1], dataSize, sizeof(sgRecvBuf) - 1);
		if (dataSize == 0)
			return false;
	}
#endif
	...
	const byte *end = src + dataSize;
	if (cmd == CMD_DLEVEL_JUNK) {
		DJunk importedJunk {};
		if (!DeltaImportJunk(src, end, importedJunk) || src != end)
			return false;
		sgJunk = importedJunk;
	} else if (cmd == CMD_DLEVEL) {
		if (src >= end)
			return false;
		const uint8_t level = static_cast<uint8_t>(*src++);
		if (level >= MAX_MULTIPLAYERLEVELS)
			return false;

		DLevel importedLevel {};
		if (!DeltaImportItem(src, end, importedLevel.item)
		    || !DeltaImportObjects(src, end, importedLevel.object)
		    || !DeltaImportMonster(src, end, importedLevel.monster)
		    || src != end) {
			return false;
		}
		GetDeltaLevel(level) = std::move(importedLevel);
	} else {
		return false;
	}
...
size_t OnLevelData(int pnum, const TCmd *pCmd, size_t availableBytes)
{
	if (availableBytes < sizeof(TCmdPlrInfoHdr))
		return 0;
	...
	if (wBytes > availableBytes - sizeof(message))
		return 0;
	...
	if (sgbRecvCmd == CMD_DLEVEL_END) {
		...
		if (!DeltaImportData(sgbRecvCmd, sgdwRecvOffset))
			return 0;
		if (message.bCmd == CMD_DLEVEL_END) {
			sgbDeltaChunks = MAX_CHUNKS - 1;
			sgbRecvCmd = CMD_DLEVEL_END;
	...
	if (wOffset != sgdwRecvOffset)
		return wBytes + sizeof(message);
	if (wOffset > sizeof(sgRecvBuf) || wBytes > sizeof(sgRecvBuf) - wOffset)
		return wBytes + sizeof(message);
...
size_t OnAttackMonster(const TCmd *pCmd, Player &player)
{
	...
	const MonsterId monsterIdx = SDL_SwapLE16(message.wParam1);
...
size_t OnMonstDamage(const TCmd *pCmd, size_t pnum)
{
	...
	const MonsterId monsterIdx = SDL_SwapLE16(message.wMon);
...
size_t OnSendPlayerInfo(const TCmd *pCmd, size_t pnum, size_t availableBytes)
{
	if (availableBytes < sizeof(TCmdPlrInfoHdr))
		return 0;
	...
	const auto &message = *reinterpret_cast<const TCmdPlrInfoHdr *>(pCmd);
	const uint16_t wBytes = SDL_SwapLE16(message.wBytes);
	const size_t messageSize = sizeof(message) + wBytes;
	if (messageSize > availableBytes)
		return 0;

	if (gbBufferMsgs == 1)
		SendPacket(pnum, &message, messageSize);
	else
...
size_t OnActivateRPortal(const TCmd *pCmd, size_t pnum)
{
	const auto &message = *reinterpret_cast<const TCmdLocParam3 *>(pCmd);
	const Point position { message.x, message.y };
	const uint16_t level = SDL_SwapLE16(message.wParam1);
	...
	if (gbBufferMsgs == 1) {
		...
		if (&player != MyPlayer) {
			if (leveltype == DTYPE_TOWN) {
				...
				bool addPortal = true;
				for (auto &missile : Missiles) {
					if (missile._mitype == MissileID::Etherealize && &Players[missile._misource] == &player) {
						addPortal = false;
						break;
					}
				}
...
size_t OnDeactivateRPortal(const TCmd *pCmd, size_t pnum)
{
	if (gbBufferMsgs == 1) {
		SendPacket(pnum, pCmd, sizeof(*pCmd));
	} else {
...
size_t OnString(const TCmd *pCmd, Player &player, size_t availableBytes)
{
	if (availableBytes <= sizeof(pCmd->bCmd))
		return 0;

	const auto *p = reinterpret_cast<const TCmdString *>(pCmd);
	const size_t maxStringBytes = std::min(availableBytes - sizeof(p->bCmd), sizeof(p->str));
	const void *terminator = memchr(p->str, '\0', maxStringBytes);
	if (terminator == nullptr)
		return 0;

	const size_t len = static_cast<const char *>(terminator) - p->str;
	if (gbBufferMsgs == 0)
		SendPlrMsg(player, p->str);

	return sizeof(p->bCmd) + len + 1;
}
...
size_t OnSetDRShield(const TCmd *pCmd, Player &player)
{
	if (gbBufferMsgs != 1)
...
size_t OnSetEShield(const TCmd *pCmd, Player &player)
{
	if (gbBufferMsgs != 1)
...
size_t OnSetHRShield(const TCmd *pCmd, Player &player)
{
	if (gbBufferMsgs != 1)
...
size_t OnSetMRShield(const TCmd *pCmd, Player &player)
{
	if (gbBufferMsgs != 1)
...
size_t OnRemoveDRShield(const TCmd *pCmd, Player &player)
{
	if (gbBufferMsgs != 1)
...
size_t OnRemoveEShield(const TCmd *pCmd, Player &player)
{
	if (gbBufferMsgs != 1)
...
size_t OnOpenGravez(const TCmd *pCmd)
{
	if (gbBufferMsgs != 1) {
		TownOpenGravez();
		InitTownTriggers();
...
void delta_sync_monster(const TSyncMonster &monsterSync, uint8_t level)
{
	...
	const MonsterId monsterId = SDL_SwapLE16(monsterSync._mndx);
	DMonsterStr &monster = GetDeltaLevel(level).monster[monsterId];
	if (monster.hitPoints == 0)
		return;
...
void NetSendCmdGolem(uint8_t mx, uint8_t my, Direction dir, uint16_t menemy, int hp, uint8_t cl)
{
	TCmdGolem cmd;
...
size_t GetFixedCmdSize(_cmd_id cmd)
{
	switch (cmd) {
	case CMD_WALKXY:
	case CMD_ATTACKXY:
	...
	case CMD_ACTIVATERPORTAL:
		return sizeof(TCmdLocParam3);
	case CMD_SPELLXY:
		return sizeof(TCmdLocParam4);
	case CMD_SPELLXYD:
		return sizeof(TCmdLocParam5);
	case CMD_SPELLID:
	case CMD_SPELLPID:
		return sizeof(TCmdParam5);
	...
	case CMD_DEACTIVATERPORTAL:
	case CMD_RETOWN:
	case CMD_FRIENDLYMODE:
	case CMD_CHEAT_EXPERIENCE:
	case CMD_SETSHIELD:
	case CMD_SETDRSHIELD:
	case CMD_SETESHIELD:
	case CMD_SETHRSHIELD:
	case CMD_SETMRSHIELD:
	case CMD_REMSHIELD:
	case CMD_REDRSHIELD:
	case CMD_REMESHIELD:
	case CMD_NAKRUL:
...
size_t ParseCmd(size_t pnum, const TCmd *pCmd, size_t availableBytes)
{
	if (availableBytes < sizeof(TCmd) || pnum >= Players.size())
		return 0;

	sbLastCmd = pCmd->bCmd;
	const size_t fixedSize = GetFixedCmdSize(static_cast<_cmd_id>(sbLastCmd));
	if (fixedSize != 0 && availableBytes < fixedSize)
		return 0;
	...
	switch (pCmd->bCmd) {
	case CMD_SYNCDATA:
		return OnSyncData(pCmd, pnum, availableBytes);
	case CMD_WALKXY:
	...
	case CMD_ACK_PLRINFO:
	case CMD_SEND_PLRINFO:
		return OnSendPlayerInfo(pCmd, pnum, availableBytes);
	case CMD_PLAYER_JOINLEVEL:
	...
	case CMD_ACTIVATERPORTAL:
		return OnActivateRPortal(pCmd, pnum);
	case CMD_DEACTIVATERPORTAL:
		return OnDeactivateRPortal(pCmd, pnum);
	case CMD_RETOWN:
	...
	case CMD_STRING:
		return OnString(pCmd, player, availableBytes);
	case CMD_FRIENDLYMODE:
	...
	case CMD_SETDRSHIELD:
		return OnSetDRShield(pCmd, player);
	case CMD_SETESHIELD:
		return OnSetEShield(pCmd, player);
	case CMD_SETHRSHIELD:
		return OnSetHRShield(pCmd, player);
	case CMD_SETMRSHIELD:
		return OnSetMRShield(pCmd, player);
	case CMD_REMSHIELD:
		return OnRemoveShield(pCmd, player);
	case CMD_REDRSHIELD:
		return OnRemoveDRShield(pCmd, player);
	case CMD_REMESHIELD:
		return OnRemoveEShield(pCmd, player);
	case CMD_SETREFLECT:
	...
	case CMD_OPENGRAVEZ:
		return OnOpenGravez(pCmd);
	//case CMD_SETAURA:
	//	return OnSetShield(pCmd, player);
	//case CMD_REAuraSHIELD:
	//	return OnRemoveShield(pCmd, player);
	...
	return OnLevelData(pnum, pCmd, availableBytes);
}
