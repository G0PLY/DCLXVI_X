...
void HandleAllPackets(size_t pnum, const byte *data, size_t size)
{
	for (unsigned offset = 0; offset < size;) {
		...
		const auto *command = reinterpret_cast<const TCmd *>(&data[offset]);

		if (command->bCmd >= CMD_DLEVEL && command->bCmd <= CMD_DLEVEL_END) {
			if (remaining < sizeof(TCmdPlrInfoHdr))
				break;

			const auto *levelData = reinterpret_cast<const TCmdPlrInfoHdr *>(command);
			const size_t payloadSize = SDL_SwapLE16(levelData->wBytes);
			if (payloadSize > remaining - sizeof(TCmdPlrInfoHdr))
				break;
		}

		size_t messageSize = ParseCmd(pnum, command, remaining);
		if (messageSize == 0 || messageSize > remaining)
			break;
...
void HandleEvents(_SNETEVENT *pEvt)
{
	...
	case EVENT_TYPE_PLAYER_CREATE_GAME: {
		if (static_cast<size_t>(pEvt->playerid) >= Players.size())
			return;
		if (pEvt->data == nullptr || pEvt->databytes < sizeof(GameData))
			return;

		const auto *gameData = static_cast<const GameData *>(pEvt->data);
	...
	case EVENT_TYPE_PLAYER_LEAVE_GAME: {
		...
		if (pEvt->data != nullptr && pEvt->databytes >= sizeof(leftReason))
			memcpy(&leftReason, pEvt->data, sizeof(leftReason));
	...
	case EVENT_TYPE_PLAYER_MESSAGE:
		if (pEvt->data == nullptr || pEvt->databytes == 0)
			return;
		if (memchr(pEvt->data, '\0', pEvt->databytes) == nullptr)
			return;
		EventPlrMsg(static_cast<char *>(pEvt->data));
		break;
	}
}
...
void multi_process_network_packets()
{
	...
	while (SNetReceiveMessage(&playerId, (void **)&pkt, &dwMsgSize)) {
		...
		if (!IsNetPlayerValid(player)) {
			if (dwMsgSize < sizeof(TPktHdr) + sizeof(TCmd))
				continue;
...
void recv_plrinfo(int pnum, const TCmdPlrInfoHdr &header, bool recv)
{
	...
	const uint16_t offset = SDL_SwapLE16(header.wOffset);
	const uint16_t bytes = SDL_SwapLE16(header.wBytes);
	if (offset > sizeof(packedPlayer) || bytes > sizeof(packedPlayer) - offset)
		return;

	if (sgwPackPlrOffsetTbl[pnum] != offset) {
	...
	memcpy(reinterpret_cast<uint8_t *>(&packedPlayer) + offset, reinterpret_cast<const uint8_t *>(&header) + sizeof(header), bytes);

	sgwPackPlrOffsetTbl[pnum] += bytes;
