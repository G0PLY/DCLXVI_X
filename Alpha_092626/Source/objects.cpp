...
bool RndLocOk(int xp, int yp)
{
	...
	if (TileHasAny(dPiece[xp][yp], TileProperties::Solid))
		return false;
	return IsNoneOf(leveltype, DTYPE_CATHEDRAL, DTYPE_CRYPT, DTYPE_CRYPTZ) || dPiece[xp][yp] <= 125 || dPiece[xp][yp] >= 143;
}
...
void AddBookLever(_object_id type, WorldTileRectangle affectedArea, _speech_id msg)
{
	...
	if (type == OBJ_BLOODBOOK) {
		...
		position = GetRandomObjectPosition({ 2, 2 });
		if (!position)
			return;
	}
...
void InitRndBarrels()
{
	...
	} else if (leveltype == DTYPE_CRYPT || leveltype == DTYPE_CRYPTZ) {
		barrelId = OBJ_URN;
...
void AddObjTraps()
{
	...
	for (int j = 0; j < MAXDUNY; j++) {
		for (int i = 0; i < MAXDUNX; i++) {
			...
			if (FlipCoin()) {
				int xp = i - 1;
				while (xp >= 0 && IsTileNotSolid({ xp, j }))
					xp--;

				if (xp < 0 || !CanPlaceWallTrap(xp, j) || i - xp <= 1)
					continue;

				trapObject = AddObject(OBJ_TRAPL, { xp, j });
			} else {
				int yp = j - 1;
				while (yp >= 0 && IsTileNotSolid({ i, yp }))
					yp--;

				if (yp < 0 || !CanPlaceWallTrap(i, yp) || j - yp <= 1)
					continue;
...
void AddCryptzObject(Object &object, int a2)
{
	if (a2 > 5) {
		Player &myPlayer = *MyPlayer;
		switch (a2) {
...
void OperateLazStand(Object &stand)
{
	...
	// Do not consume the stand until we know the quest item has a valid tile.
	// GetSuperItemLoc currently returns { 0, 0 } when no valid tile is found.
	stand._oAnimFrame++;
	stand._oSelFlag = 0;
...
void AddCryptzObjects(int x1, int y1, int x2, int y2)
{
	for (int j = y1; j < y2; j++) {
		for (int i = x1; i < x2; i++) {
			int pn = dPiece[i][j];
