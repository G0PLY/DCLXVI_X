...
// Reuse the legacy 4-byte _mAnimData pointer slot in each saved monster record.
// The upper 16 bits tag saves written by this extended build; the lower 16 bits
// store the signed 16-bit global monster ID without changing the record size.
constexpr uint32_t ExtendedMonsterTypeTag = 0x44550000U; // "DU"
constexpr uint32_t ExtendedMonsterTypeTagMask = 0xFFFF0000U;
constexpr uint32_t ExtendedMonsterTypeValueMask = 0x0000FFFFU;

/** Number of monster kill counters reserved by the legacy save layout. */
constexpr size_t LegacyMonsterKillCountSlots = 255;

/** Number of live-monster slots serialized by the legacy save layout. */
constexpr size_t LegacyMonsterInstanceSlots = 255;

/** Stage 5-9 extended saves serialize a fixed 512-entry ActiveMonsters block. */
constexpr size_t ExtendedMonsterInstanceSlotsV1 = 512;

/** Stage 10 extended saves can serialize a fixed 1024-entry ActiveMonsters block. */
constexpr size_t ExtendedMonsterInstanceSlotsV2 = 1024;

static_assert(LegacyMonsterInstanceSlots <= ExtendedMonsterInstanceSlotsV1);
static_assert(ExtendedMonsterInstanceSlotsV1 <= ExtendedMonsterInstanceSlotsV2);
static_assert(ExtendedMonsterInstanceSlotsV2 <= MaxMonsters, "Extended monster slot block exceeds runtime capacity");

// Preserve both historical layouts:
//   count >= 0      -> legacy 255-slot ActiveMonsters block
//   -1 .. -512      -> Stage 5-9 512-slot block
//   INT32_MIN+count -> Stage 10 1024-slot block (used when count > 512)
//
// The V2 marker is deliberately far outside the V1 range, so Stage 5-9 saves
// remain readable after MaxMonsters grows from 512 to 1024.
constexpr int32_t ExtendedMonsterCountV2Base = std::numeric_limits<int32_t>::min();

constexpr size_t MonsterSlotsToSave(size_t count)
{
	if (count <= LegacyMonsterInstanceSlots)
		return LegacyMonsterInstanceSlots;
	if (count <= ExtendedMonsterInstanceSlotsV1)
		return ExtendedMonsterInstanceSlotsV1;
	return ExtendedMonsterInstanceSlotsV2;
}

constexpr int32_t EncodeActiveMonsterCount(size_t count)
{
	if (count <= LegacyMonsterInstanceSlots)
		return static_cast<int32_t>(count);
	if (count <= ExtendedMonsterInstanceSlotsV1)
		return -static_cast<int32_t>(count);
	return ExtendedMonsterCountV2Base + static_cast<int32_t>(count);
}

size_t DecodeActiveMonsterCount(int32_t savedCount, size_t &count)
{
	int64_t decodedCount;
	size_t savedMonsterSlots;

	if (savedCount >= 0) {
		decodedCount = savedCount;
		savedMonsterSlots = LegacyMonsterInstanceSlots;
	} else if (savedCount >= -static_cast<int32_t>(ExtendedMonsterInstanceSlotsV1)) {
		decodedCount = -static_cast<int64_t>(savedCount);
		savedMonsterSlots = ExtendedMonsterInstanceSlotsV1;
	} else {
		decodedCount = static_cast<int64_t>(savedCount) - static_cast<int64_t>(ExtendedMonsterCountV2Base);
		if (decodedCount <= static_cast<int64_t>(ExtendedMonsterInstanceSlotsV1)
		    || decodedCount > static_cast<int64_t>(ExtendedMonsterInstanceSlotsV2))
			app_fatal(_("Invalid monster count in save file"));
		savedMonsterSlots = ExtendedMonsterInstanceSlotsV2;
	}

	if (decodedCount < 0 || decodedCount > static_cast<int64_t>(MaxMonsters))
		app_fatal(_("Invalid monster count in save file"));

	count = static_cast<size_t>(decodedCount);
	return savedMonsterSlots;
}

// The original save-game layout reserves exactly 255 int32_t monster-kill
// counters. Monster definitions may now grow beyond that legacy block.
// IDs 0..254 retain their original persisted counters; counters for higher
// monster IDs remain runtime-only until an extended save block is introduced.
constexpr size_t PersistedMonsterKillCountSlots()
{
	return std::min(static_cast<size_t>(NUM_MTYPES), LegacyMonsterKillCountSlots);
}
...
class LoadHelper {
	...
	T Next()
	{
		...
		if (!IsValid(size))
			app_fatal(_("Unexpected end of save file"));
...
public:
	...
	{
		if (!IsValid(len))
			app_fatal(_("Save buffer overflow"));
...
void LoadActiveMonsterSlots(LoadHelper &file, size_t savedMonsterSlots, size_t activeMonsterCount)
{
	for (size_t i = 0; i < savedMonsterSlots; ++i) {
		const int32_t savedMonsterId = file.NextBE<int32_t>();
		if (savedMonsterId < 0 || savedMonsterId >= static_cast<int32_t>(MaxMonsters))
			app_fatal(_("Invalid monster index in save file"));
		ActiveMonsters[i] = static_cast<MonsterId>(savedMonsterId);
	}

	for (size_t i = savedMonsterSlots; i < MaxMonsters; ++i)
		ActiveMonsters[i] = static_cast<MonsterId>(i);

	std::array<bool, MaxMonsters> seenMonsterIds {};
	for (size_t i = 0; i < activeMonsterCount; ++i) {
		const MonsterId monsterId = ActiveMonsters[i];
		if (seenMonsterIds[monsterId])
			app_fatal(_("Duplicate active monster index in save file"));
...
void LoadItemData(LoadHelper &file, Item &item)
{
	...
	const int32_t itemType = file.NextLE<int32_t>();
	if (itemType < static_cast<int32_t>(ItemType::None) || itemType > static_cast<int32_t>(ItemType::Amulet))
		app_fatal(_("Invalid item type in save file"));
	item._itype = static_cast<ItemType>(itemType);
	...
	const int8_t itemQuality = file.NextLE<int8_t>();
	if (itemQuality < ITEM_QUALITY_NORMAL || itemQuality > ITEM_QUALITY_UNIQUE)
		app_fatal(_("Invalid item quality in save file"));
	item._iMagical = static_cast<item_quality>(itemQuality);
	file.NextBytes(item._iName, 64);
	file.NextBytes(item._iIName, 64);
	const int8_t itemLocation = file.NextLE<int8_t>();
	if (itemLocation < ILOC_INVALID || itemLocation > ILOC_BELT)
		app_fatal(_("Invalid item equipment location in save file"));
	item._iLoc = static_cast<item_equip_type>(itemLocation);
	const uint8_t itemClass = file.NextLE<uint8_t>();
	if (itemClass > ICLASS_QUEST)
		app_fatal(_("Invalid item class in save file"));
	item._iClass = static_cast<item_class>(itemClass);
	...
	const int32_t itemMiscId = file.NextLE<int32_t>();
	if (itemMiscId < IMISC_INVALID || itemMiscId > IMISC_ARENAPOT)
		app_fatal(_("Invalid item miscellaneous type in save file"));
	item._iMiscId = static_cast<item_misc_id>(itemMiscId);
	const int32_t itemSpell = file.NextLE<int32_t>();
	if (itemSpell < static_cast<int32_t>(SpellID::Invalid) || itemSpell > static_cast<int32_t>(SpellID::LAST))
		app_fatal(_("Invalid item spell in save file"));
	item._iSpell = static_cast<SpellID>(itemSpell);
	...
	const int32_t savedItemIndex = file.NextLE<int32_t>();
	if (savedItemIndex < IDI_NONE || savedItemIndex > IDI_LAST)
		app_fatal(_("Invalid item index in save file"));
	item.IDidx = static_cast<_item_indexes>(savedItemIndex);
	if (gbIsSpawn) {
...
{
	const int32_t playerMode = file.NextLE<int32_t>();
	if (playerMode < PM_STAND || playerMode > PM_SATTACK)
		app_fatal(_("Invalid player mode in save file"));
	player._pmode = static_cast<PLR_MODE>(playerMode);
	...
	file.Skip(2); // Alignment
	const int32_t destinationAction = file.NextLE<int32_t>();
	...
	case ACTION_SATTACK:
		break;
	default:
		app_fatal(_("Invalid player destination action in save file"));
	}
	player.destAction = static_cast<action_id>(destinationAction);
	...
	const int32_t queuedSpellId = file.NextLE<int32_t>();
	if (queuedSpellId < static_cast<int32_t>(SpellID::Invalid) || queuedSpellId > static_cast<int32_t>(SpellID::LAST))
		app_fatal(_("Invalid queued spell in save file"));
	player.queuedSpell.spellId = static_cast<SpellID>(queuedSpellId);
	const int8_t queuedSpellType = file.NextLE<int8_t>();
	if (queuedSpellType < static_cast<int8_t>(SpellType::FIRST) || queuedSpellType > static_cast<int8_t>(SpellType::Invalid))
		app_fatal(_("Invalid queued spell type in save file"));
	player.queuedSpell.spellType = static_cast<SpellType>(queuedSpellType);
	...
	file.Skip(2); // Alignment
	const int32_t inventorySpell = file.NextLE<int32_t>();
	if (inventorySpell < static_cast<int32_t>(SpellID::Invalid) || inventorySpell > static_cast<int32_t>(SpellID::LAST))
		app_fatal(_("Invalid inventory spell in save file"));
	player.inventorySpell = static_cast<SpellID>(inventorySpell);
	file.Skip<int8_t>(); // Skip _pTSplType
	file.Skip(3);        // Alignment
	const int32_t readySpell = file.NextLE<int32_t>();
	if (readySpell < static_cast<int32_t>(SpellID::Invalid) || readySpell > static_cast<int32_t>(SpellID::LAST))
		app_fatal(_("Invalid ready spell in save file"));
	player._pRSpell = static_cast<SpellID>(readySpell);
	const int8_t readySpellType = file.NextLE<int8_t>();
	if (readySpellType < static_cast<int8_t>(SpellType::FIRST) || readySpellType > static_cast<int8_t>(SpellType::Invalid))
		app_fatal(_("Invalid ready spell type in save file"));
	player._pRSplType = static_cast<SpellType>(readySpellType);
	file.Skip(3); // Alignment
	const int32_t spellbookSpell = file.NextLE<int32_t>();
	if (spellbookSpell < static_cast<int32_t>(SpellID::Invalid) || spellbookSpell > static_cast<int32_t>(SpellID::LAST))
		app_fatal(_("Invalid spellbook spell in save file"));
	player._pSBkSpell = static_cast<SpellID>(spellbookSpell);
	...
	for (size_t i = 0; i < 4; i++) {
		const int32_t hotkeySpell = file.NextLE<int32_t>();
		if (hotkeySpell < static_cast<int32_t>(SpellID::Invalid) || hotkeySpell > static_cast<int32_t>(SpellID::LAST))
			app_fatal(_("Invalid hotkey spell in save file"));
		player._pSplHotKey[i] = static_cast<SpellID>(hotkeySpell);
	}
	for (size_t i = 0; i < 4; i++) {
		const uint8_t hotkeySpellType = file.NextLE<uint8_t>();
		if (hotkeySpellType > static_cast<uint8_t>(SpellType::Invalid))
			app_fatal(_("Invalid hotkey spell type in save file"));
		player._pSplTHotKey[i] = static_cast<SpellType>(hotkeySpellType);
	}
	...
	if (player._pClass > HeroClass::LAST)
		app_fatal(_("Invalid player class in save file"));
	file.Skip(3); // Alignment
	...
	if (player._pNumInv < 0 || player._pNumInv > InventoryGridCells)
		app_fatal(_("Invalid inventory count in save file"));
	...
	player.pDmgReduct = file.NextBool8();
	player.pEtherShield = file.NextBool8();
	player.pHlthregn = file.NextBool8();
	player.pManaregn = file.NextBool8();
	if (gbIsHellfireSaveGame) {
	...
	const uint32_t difficulty = file.NextLE<uint32_t>();
	if (difficulty > static_cast<uint32_t>(DIFF_LAST))
		app_fatal(_("Invalid difficulty in save file"));
	...
	player._pClasstype = file.NextLE<int8_t>();
...
void LoadMonster(LoadHelper *file, Monster &monster)
{
	...
	monster.enemy = static_cast<MonsterId>(file->NextLE<int32_t>());
	monster.enemyPosition.x = file->NextLE<uint8_t>();
	monster.enemyPosition.y = file->NextLE<uint8_t>();
	file->Skip(2); // Unused

	const uint32_t savedMonsterTypeTag = file->NextLE<uint32_t>();
	const bool hasExtendedMonsterTypeTag = (savedMonsterTypeTag & ExtendedMonsterTypeTagMask) == ExtendedMonsterTypeTag;
	if (hasExtendedMonsterTypeTag) {
		const auto savedMonsterType = static_cast<_monster_id>(static_cast<int16_t>(savedMonsterTypeTag & ExtendedMonsterTypeValueMask));
		if (savedMonsterType < 0 || savedMonsterType >= NUM_MTYPES)
			app_fatal(_("Invalid monster type in save file"));
		monster.levelType = AddMonsterType(savedMonsterType, PLACE_SCATTER);
	}
	if (monster.levelType >= LevelMonsterTypeCount)
		app_fatal(_("Invalid level monster type in save file"));
	monster.animInfo = {};
	...
	const uint8_t leaderHighByte = file->NextLE<uint8_t>(); // Extended build reuses one legacy alignment byte.
	file->Skip(1); // Remaining alignment byte
	...
	const uint8_t leaderLowByte = file->NextLE<uint8_t>();
	monster.leader = hasExtendedMonsterTypeTag
	    ? static_cast<MonsterId>((static_cast<uint16_t>(leaderHighByte) << 8) | leaderLowByte)
	    : static_cast<MonsterId>(leaderLowByte);
	if (monster.leader == 0)
		monster.leader = Monster::NoLeader; // Golems shouldn't be leaders of other monsters
	else if (monster.leader >= MaxMonsters)
		app_fatal(_("Invalid monster leader index in save file"));
...
{
	Missile missile = {};
	const int32_t missileType = file->NextLE<int32_t>();
	if (missileType < static_cast<int32_t>(MissileID::Null) || missileType > static_cast<int32_t>(MissileID::QuickSxpl))
		app_fatal(_("Invalid missile type in save file"));
	missile._mitype = static_cast<MissileID>(missileType);
	...
	if (missile._miAnimType > MissileGraphicID::None)
		app_fatal(_("Invalid missile graphic in save file"));
	file->Skip(3); // Alignment
	...
	if (missile._micaster > TARGET_BOTH)
		app_fatal(_("Invalid missile caster in save file"));
	if (missile._misource < -1)
		app_fatal(_("Invalid missile source in save file"));
	if (missile._misource != -1) {
		if (missile._micaster == TARGET_PLAYERS) {
			if (static_cast<size_t>(missile._misource) >= MaxMonsters)
				app_fatal(_("Invalid monster missile source in save file"));
		} else if (static_cast<size_t>(missile._misource) >= Players.size()) {
			app_fatal(_("Invalid player missile source in save file"));
		}
	}
...
_object_id ConvertFromHellfireObject(_object_id type)
{
	...
	if (leveltype == DTYPE_CRYPT || leveltype == DTYPE_CRYPTZ) {
		switch (type) {
		case OBJ_BARREL:
			return OBJ_URN;
		case OBJ_BARRELEX:
...
{
	const int32_t objectType = file.NextLE<int32_t>();
	if (objectType < static_cast<int32_t>(OBJ_NULL) || objectType > static_cast<int32_t>(OBJ_L5SARC))
		app_fatal(_("Invalid object type in save file"));
	object._otype = ConvertFromHellfireObject(static_cast<_object_id>(objectType));
	...
	const int32_t bookMessage = file.NextLE<int32_t>();
	if (bookMessage < static_cast<int32_t>(TEXT_NONE) || bookMessage > static_cast<int32_t>(TEXT_BBOOKC))
		app_fatal(_("Invalid object speech ID in save file"));
	object.bookMessage = static_cast<_speech_id>(bookMessage);
...
void LoadQuest(LoadHelper *file, int i)
{
	...
	if (quest._qactive != QUEST_NOTAVAIL
	    ...
	    && quest._qactive != QUEST_INVALID)
		app_fatal(_("Invalid quest state in save file"));
	quest._qlvltype = static_cast<dungeon_type>(file->NextLE<uint8_t>());
	if (quest._qlvltype < DTYPE_TOWN || quest._qlvltype > DTYPE_LAST)
		app_fatal(_("Invalid quest level type in save file"));
	...
	if (quest._qslvl < SL_NONE || quest._qslvl > SL_LAST)
		app_fatal(_("Invalid quest set level in save file"));
	const uint8_t questId = file->NextLE<uint8_t>();
	if (questId >= MAXQUESTS)
		app_fatal(_("Invalid quest ID in save file"));
	quest._qidx = static_cast<quest_id>(questId);
	if (gbIsHellfireSaveGame) {
		file->Skip(2); // Alignment
	...
	if (ReturnLevel < 0 || ReturnLevel >= giNumberOfLevels)
		app_fatal(_("Invalid quest return level in save file"));
	const int32_t returnLevelType = file->NextBE<int32_t>();
	if (returnLevelType < static_cast<int32_t>(DTYPE_TOWN) || returnLevelType > static_cast<int32_t>(DTYPE_LAST))
		app_fatal(_("Invalid quest return level type in save file"));
	ReturnLevelType = static_cast<dungeon_type>(returnLevelType);
...
void LoadPortal(LoadHelper *file, int i)
{
	...
	if (pPortal->level < 0 || pPortal->level >= giNumberOfLevels)
		app_fatal(_("Invalid portal level in save file"));
	if (pPortal->ltype < DTYPE_TOWN || pPortal->ltype > DTYPE_LAST)
		app_fatal(_("Invalid portal level type in save file"));
...
void LoadDroppedItems(LoadHelper &file, size_t savedItemCount)
{
	if (savedItemCount > MAXITEMS)
		app_fatal(_("Invalid item count in save file"));
	...
	for (size_t i = 0; i < savedItemCount; i++) {
		...
		if (!item.isEmpty()) {
			if (item.position.x < 0 || item.position.x >= MAXDUNX || item.position.y < 0 || item.position.y >= MAXDUNY)
				app_fatal(_("Invalid item position in save file"));
			// Loaded a valid item
			ActiveItemCount++;
...
{
	if (type == DTYPE_CRYPT || leveltype == DTYPE_CRYPTZ)
...
void SavePlayer(SaveHelper &file, const Player &player)
{
	...
	file.WriteLE<uint8_t>(player.pDmgReduct ? 1 : 0);
	file.WriteLE<uint8_t>(player.pEtherShield ? 1 : 0);
	file.WriteLE<uint8_t>(player.pHlthregn ? 1 : 0);
	file.WriteLE<uint8_t>(player.pManaregn ? 1 : 0);
	...
	file.WriteLE<int8_t>(player._pClasstype);
	file.Skip(20); // Available bytes
...
void SaveMonster(SaveHelper *file, Monster &monster)
{
	...
	file->WriteLE<int32_t>(static_cast<int32_t>(monster.enemy));
	file->WriteLE<uint8_t>(monster.enemyPosition.x);
	file->WriteLE<uint8_t>(monster.enemyPosition.y);
	file->Skip(2); // Unused

	const uint32_t savedMonsterTypeTag = ExtendedMonsterTypeTag
	    | static_cast<uint16_t>(monster.type().type);
	file->WriteLE<uint32_t>(savedMonsterTypeTag);
	...
	const MonsterId savedLeader = monster.leader == Monster::NoLeader ? 0 : monster.leader;
	file->WriteLE<uint8_t>(static_cast<uint8_t>(savedLeader >> 8)); // Reuse one legacy alignment byte for leader IDs > 255.
	file->Skip(1); // Remaining alignment byte

	file->WriteLE<int32_t>(monster.talkMsg == TEXT_NONE ? 0 : monster.talkMsg);       // Replicate original bad mapping of none for monsters
	file->WriteLE<uint8_t>(static_cast<uint8_t>(savedLeader)); // Low byte remains in the vanilla leader field.
...
void LoadAdditionalMissiles()
{
	...
	const size_t remainingMissileCapacity = Missiles.max_size() - Missiles.size();
	if (missileCountAdditional > remainingMissileCapacity)
		app_fatal(_("Invalid additional missile count in save file"));
...
{
	// IDidx is save-derived and may be changed by legacy Spawn/Diablo remapping.
	// Validate the final runtime domain before passing it to item-table helpers.
	if (item.IDidx < IDI_GOLD || item.IDidx > IDI_LAST) {
		item.clear();
		return;
	}
...
void LoadStash()
{
	...
	constexpr uint32_t MaxStashPages = 100;
	constexpr uint32_t MaxStashItems = MaxStashPages * 100;
	if (pages > MaxStashPages)
		app_fatal(_("Invalid stash page count in save file"));
	for (unsigned i = 0; i < pages; i++) {
		auto page = file.NextLE<uint32_t>();
		if (page >= MaxStashPages)
			app_fatal(_("Invalid stash page in save file"));
	...
	if (itemCount > MaxStashItems)
		app_fatal(_("Invalid stash item count in save file"));
	Stash.stashList.resize(itemCount);
	for (unsigned i = 0; i < itemCount; i++) {
		LoadItemData(file, Stash.stashList[i]);
	}

	const uint32_t currentPage = file.NextLE<uint32_t>();
	if (currentPage >= MaxStashPages)
		app_fatal(_("Invalid current stash page in save file"));
	Stash.SetPage(currentPage);
}
...
void LoadGame(bool firstflag)
{
	...
	if (currlevel >= giNumberOfLevels)
		app_fatal(_("Invalid level in save file"));
	if (setlevel) {
		if (setlvlnum < SL_NONE || setlvlnum > SL_LAST)
			app_fatal(_("Invalid set level in save file"));
		if (leveltype < DTYPE_TOWN || leveltype > DTYPE_LAST)
			app_fatal(_("Invalid level type in save file"));
	} else {
		leveltype = GetLevelType(currlevel);
	}
	...
	size_t tmpNummonsters;
	const size_t savedMonsterSlots = DecodeActiveMonsterCount(savedMonsterCount, tmpNummonsters);
	...
	if (savedItemCount > MAXITEMS)
		app_fatal(_("Invalid item count in save file"));
	if (tmpNummissiles < 0 || tmpNummissiles > static_cast<int>(MaxMissilesForSaveGame))
		app_fatal(_("Invalid missile count in save file"));
	if (tmpNobjects < 0 || tmpNobjects > static_cast<int>(std::size(ActiveObjects)))
		app_fatal(_("Invalid object count in save file"));

	if (!gbIsHellfire && IsAnyOf(leveltype, DTYPE_NEST, DTYPE_CRYPT, DTYPE_CRYPTZ))
	...
	for (size_t i = 0; i < PersistedMonsterKillCountSlots(); ++i)
		MonsterKillCounts[i] = file.NextBE<int32_t>();

	// Preserve the original 255-counter save-game block. When NUM_MTYPES is
	// below 255, skip the unused legacy slots. When it is above 255, the
	// additional runtime counters intentionally consume no legacy save bytes.
	file.Skip(4 * (LegacyMonsterKillCountSlots - PersistedMonsterKillCountSlots()));
	if (leveltype != DTYPE_TOWN) {
		LoadActiveMonsterSlots(file, savedMonsterSlots, ActiveMonsterCount);
		for (size_t i = 0; i < ActiveMonsterCount; i++)
		...
		for (int i = 0; i < ActiveObjectCount; i++) {
			if (ActiveObjects[i] < 0 || ActiveObjects[i] >= static_cast<int>(std::size(Objects)))
				app_fatal(_("Invalid object index in save file"));
		...
		if (ActiveLightCount < 0 || ActiveLightCount > static_cast<int>(std::size(ActiveLights)))
			app_fatal(_("Invalid light count in save file"));

		for (uint8_t &lightId : ActiveLights)
			lightId = file.NextLE<uint8_t>();
		for (int i = 0; i < ActiveLightCount; i++) {
			if (ActiveLights[i] >= std::size(Lights))
				app_fatal(_("Invalid light index in save file"));
			LoadLighting(&file, &Lights[ActiveLights[i]]);
		}

		file.Skip<int32_t>(); // VisionId
		int visionCount = file.NextBE<int32_t>();
		if (visionCount < 0 || visionCount > static_cast<int>(std::size(VisionList)))
			app_fatal(_("Invalid vision count in save file"));
	...
	for (int j = 0; j < MAXDUNY; j++) {
		for (int i = 0; i < MAXDUNX; i++) { // NOLINT(modernize-loop-convert)
			const int8_t playerCell = file.NextLE<int8_t>();
			if (playerCell < -MAX_PLRS || playerCell > MAX_PLRS)
				app_fatal(_("Invalid player grid index in save file"));
			dPlayer[i][j] = playerCell;
		}
	}
	...
	if (leveltype != DTYPE_TOWN) {
		for (int j = 0; j < MAXDUNY; j++) {
			for (int i = 0; i < MAXDUNX; i++) { // NOLINT(modernize-loop-convert)
				...
				if (monsterCell < -static_cast<int32_t>(MaxMonsters) || monsterCell > static_cast<int32_t>(MaxMonsters))
					app_fatal(_("Invalid monster grid index in save file"));
				dMonster[i][j] = static_cast<int16_t>(monsterCell);
			}
		}
		for (int j = 0; j < MAXDUNY; j++) {
			for (int i = 0; i < MAXDUNX; i++) // NOLINT(modernize-loop-convert)
		...
		for (int j = 0; j < MAXDUNY; j++) {
			for (int i = 0; i < MAXDUNX; i++) { // NOLINT(modernize-loop-convert)
				...
				const int objectLimit = static_cast<int>(std::size(Objects));
				if (objectCell < -objectLimit || objectCell > objectLimit)
					app_fatal(_("Invalid object grid index in save file"));
				dObject[i][j] = objectCell;
			}
		}
		file.Skip<uint8_t>(MAXDUNY * MAXDUNX); // dLight
		for (int j = 0; j < MAXDUNY; j++) {
	...
	for (auto &missile : Missiles) {
		if (missile._mitype == MissileID::DmgReduct && !missile._miDelFlag) {
			if (missile._misource < 0 || static_cast<size_t>(missile._misource) >= Players.size())
				app_fatal(_("Invalid missile source in save file"));
			Players[missile._misource].pDmgReduct = true;
			missile._miDelFlag = true;
		}
	}
	for (auto &missile : Missiles) {
		if (missile._mitype == MissileID::Etherealize && !missile._miDelFlag) {
			if (missile._misource < 0 || static_cast<size_t>(missile._misource) >= Players.size())
				app_fatal(_("Invalid missile source in save file"));
			Players[missile._misource].pEtherShield = true;
			missile._miDelFlag = true;
		}
	}
	for (auto &missile : Missiles) {
		if (missile._mitype == MissileID::HealthRegen && !missile._miDelFlag) {
			if (missile._misource < 0 || static_cast<size_t>(missile._misource) >= Players.size())
				app_fatal(_("Invalid missile source in save file"));
			Players[missile._misource].pHlthregn = true;
			missile._miDelFlag = true;
		}
	}
	for (auto &missile : Missiles) {
		if (missile._mitype == MissileID::ManaRegen && !missile._miDelFlag) {
			if (missile._misource < 0 || static_cast<size_t>(missile._misource) >= Players.size())
				app_fatal(_("Invalid missile source in save file"));
			Players[missile._misource].pManaregn = true;
			missile._miDelFlag = true;
		}
	} /**/
...
{
	SaveHelper file(saveWriter, "game", 512 * 1024);
	...
	file.WriteBE<int32_t>(EncodeActiveMonsterCount(ActiveMonsterCount));
	...
	for (int i = 0; i < MAXPORTAL; i++)
		SavePortal(&file, i);
	for (size_t i = 0; i < PersistedMonsterKillCountSlots(); ++i)
		file.WriteBE<int32_t>(MonsterKillCounts[i]);

	// Keep the legacy block exactly 255 counters wide so every following save
	// field stays at its historical offset regardless of NUM_MTYPES.
	file.Skip(4 * (LegacyMonsterKillCountSlots - PersistedMonsterKillCountSlots()));

	if (leveltype != DTYPE_TOWN) {
		const size_t savedMonsterSlots = MonsterSlotsToSave(ActiveMonsterCount);
		for (size_t i = 0; i < savedMonsterSlots; ++i)
			file.WriteBE<int32_t>(static_cast<int32_t>(ActiveMonsters[i]));
		for (size_t i = 0; i < ActiveMonsterCount; i++)
...
void LoadLevel()
{
	...
	const int32_t savedMonsterCount = file.NextBE<int32_t>();
	const size_t savedMonsterSlots = DecodeActiveMonsterCount(savedMonsterCount, ActiveMonsterCount);
	auto savedItemCount = file.NextBE<uint32_t>();
	ActiveObjectCount = file.NextBE<int32_t>();
	if (savedItemCount > MAXITEMS)
		app_fatal(_("Invalid item count in save file"));
	if (ActiveObjectCount < 0 || ActiveObjectCount > static_cast<int>(std::size(ActiveObjects)))
		app_fatal(_("Invalid object count in save file"));
